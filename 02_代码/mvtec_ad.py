"""MVTec AD directory indexing and validation splitting."""
...
class MVTecADIndex:
    def __init__(self, root):
        ...
        if not self.root.exists():
            raise FileNotFoundError(f"MVTec AD root does not exist: {self.root}")
    ...
    def samples(self, category, split):
        ...
        if not split_root.is_dir():
            raise FileNotFoundError(f"missing MVTec AD split: {split_root}")
        records = []
        for defect_root in sorted(path for path in split_root.iterdir() if path.is_dir()):
            is_anomaly = defect_root.name != "good"
            for image_path in sorted(path for path in defect_root.iterdir() if path.suffix.lower() in IMAGE_SUFFIXES):
                mask_path = None
                if is_anomaly:
                    candidate = category_root / "ground_truth" / defect_root.name / f"{image_path.stem}_mask{image_path.suffix}"
                    if candidate.exists():
                        mask_path = str(candidate)
                    else:
                        png_candidate = candidate.with_suffix(".png")
                        mask_path = str(png_candidate) if png_candidate.exists() else None
                records.append({
