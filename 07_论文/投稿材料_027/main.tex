...
This study uses public image datasets and does not involve human participants, animals or personally identifiable information. The main submission package includes the split protocol, source data for the figures, unit-level result files and evaluation scripts. Public dataset sources are cited in the reference list. The final public repository URL will be inserted before submission; until then, the local archive path is maintained in the project record.
...
\textbf{Data availability.} MVTec AD, MPDD and VisA are publicly available benchmark datasets. The derived category--seed result tables and figure source data will be deposited in a public repository before submission; the repository identifier is an author action item and is intentionally not fabricated here.

\textbf{Code availability.} The evaluation scripts, configuration files and route definitions will be released with the submission package and linked from the public repository before submission.
